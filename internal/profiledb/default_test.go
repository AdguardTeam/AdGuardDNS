...
// type check
var _ websvc.CertificateValidator = (*profiledb.Default)(nil)
...
func TestDefault_IsValidWellKnownRequest(t *testing.T) {
	t.Parallel()

	ps := agdtest.NewProfileStorage()

	var storageProfile *agd.Profile
	ps.OnProfiles = func(
		_ context.Context,
		req *profiledb.StorageProfilesRequest,
	) (resp *profiledb.StorageProfilesResponse, err error) {
		return &profiledb.StorageProfilesResponse{
			Profiles: []*agd.Profile{storageProfile},
		}, nil
	}
	...
	httpReq := httptest.NewRequest(http.MethodGet, profiledbtest.WellKnownPath, nil)

	require.True(t, t.Run("initial", func(t *testing.T) {
		storageProfile, _ = profiledbtest.NewProfile(t)
		storageProfile.CustomDomains.Domains = []*agd.CustomDomainConfig{}
	...
	require.True(t, t.Run("new_profile", func(t *testing.T) {
		...
		storageProfile.CustomDomains.Domains = []*agd.CustomDomainConfig{{
			State: &agd.CustomDomainStatePending{
				...
				WellKnownPath: profiledbtest.WellKnownPath,
			},
		}}
	...
	require.True(t, t.Run("profile_cert_expired", func(t *testing.T) {
		storageProfile, _ = profiledbtest.NewProfile(t)
		storageProfile.CustomDomains.Domains = []*agd.CustomDomainConfig{{
			State: &agd.CustomDomainStatePending{
				Expire:        time.Now().Add(-1 * time.Hour),
	...
	require.True(t, t.Run("profile_cert_disabled", func(t *testing.T) {
		storageProfile, _ = profiledbtest.NewProfile(t)
		storageProfile.CustomDomains.Enabled = false
		storageProfile.CustomDomains.Domains = []*agd.CustomDomainConfig{{
			State: &agd.CustomDomainStatePending{
				Expire:        time.Now().Add(1 * time.Hour),
				WellKnownPath: profiledbtest.WellKnownPath,
...
func TestDefault_IsValidWellKnownRequest_expiredLater(t *testing.T) {
	t.Parallel()
	...
	// NOTE:  Now is called multiple times during the initialization and
	// refreshes.
	nowCh := make(chan time.Time, 3)
	...
	ps.OnProfiles = func(
		...
		p, _ := profiledbtest.NewProfile(t)
		p.CustomDomains.Domains = []*agd.CustomDomainConfig{{
			State: &agd.CustomDomainStatePending{
				Expire:        expire,
				WellKnownPath: profiledbtest.WellKnownPath,
			},
