...
// toInternal converts filter config's protobuf typosquatting filter structure
// to an internal one.  If x is nil, returns a disabled config.
...
// typosquattingToProtobuf converts typosquatting configuration to protobuf.  c
// must not be nil.
func typosquattingToProtobuf(
	c *filter.ConfigTyposquatting,
) (pb *fcpb.FilterConfig_SafeBrowsing_TyposquattingFilter) {
	return fcpb.FilterConfig_SafeBrowsing_TyposquattingFilter_builder{
		Enabled: c.Enabled,
	}.Build()
}
