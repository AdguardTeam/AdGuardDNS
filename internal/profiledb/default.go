...
type Default struct {
	...
	// wellKnownPathToExpire maps well-known paths for certificate validation to
...
func New(c *Config) (db *Default, err error) {
	...
	db = &Default{
		...
		wellKnownPathToExpire: make(map[string]time.Time),
...
func (db *Default) setProfileCerts(p *agd.Profile) {
	if !p.CustomDomains.Enabled {
	...
	now := db.clock.Now()
	for _, c := range p.CustomDomains.Domains {
		s, ok := c.State.(*agd.CustomDomainStatePending)
		if !ok {
			continue
		}
		...
		if s.Expire.Before(now) {
			...
			delete(db.wellKnownPathToExpire, s.WellKnownPath)

			continue
		}

		db.wellKnownPathToExpire[s.WellKnownPath] = s.Expire
	}
}
...
func (db *Default) IsValidWellKnownRequest(ctx context.Context, r *http.Request) (ok bool) {
	db.mapsMu.RLock()
	defer db.mapsMu.RUnlock()
...
func (db *Default) removeWellKnownPath(ctx context.Context, wkPath string) {
	defer slogutil.RecoverAndExit(ctx, db.logger, osutil.ExitCodeFailure)
