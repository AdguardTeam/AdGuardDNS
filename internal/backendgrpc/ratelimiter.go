...
type RateLimiter struct {
	...
	allowlist   *ratelimit.DynamicAllowlist
	errColl     errcoll.Interface
	client      dnspb.RateLimitServiceClient
	apiKey      string
}
...
func NewRateLimiter(c *RateLimiterConfig) (l *RateLimiter, err error) {
	...
	return &RateLimiter{
		...
		errColl:     c.ErrColl,
		client:      dnspb.NewRateLimitServiceClient(client),
		apiKey:      c.APIKey,
	}, nil
}
...
func (l *RateLimiter) Refresh(ctx context.Context) (err error) {
	...
	backendResp, err := l.client.GetRateLimitSettings(ctx, &dnspb.RateLimitSettingsRequest{})
	if err != nil {
		return fmt.Errorf(
	...
	prefixes := dnspb.CIDRRangeToInternal(ctx, l.logger, allowedSubnets, l.errColl)
