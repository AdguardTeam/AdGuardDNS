...
type TicketStorage struct {
	...
	grpcMetrics GRPCMetrics
	metrics     TicketStorageMetrics
	client      dnspb.SessionTicketServiceClient
	clock       timeutil.Clock
	apiKey      string
}
...
func NewSessionTicketStorage(c *TicketStorageConfig) (ts *TicketStorage, err error) {
	...
	return &TicketStorage{
		...
		metrics:     c.Metrics,
		client:      dnspb.NewSessionTicketServiceClient(cli),
		clock:       c.Clock,
		apiKey:      c.APIKey,
	}, nil
}
...
func (ts *TicketStorage) Tickets(
	...
	pbTickets := resp.GetTickets()
	results, err := dnspb.TicketsToInternal(ctx, pbTickets)
	ts.logger.DebugContext(ctx, "loaded session tickets", "count", len(results))

	tickets = make(map[tlsconfig.SessionTicketName]tlsconfig.SessionTicket, len(results))
	for name, res := range results {
		if res.Error != nil {
			ts.metrics.SetTicketStatus(ctx, string(name), startTime, res.Error)

			continue
		}

		tickets[name] = res.Ticket
	}
